...
基于日报自动生成周报和月报的全栈应用。前端 React + Vite，后端 Python FastAPI，SQLite 数据库。支持多用户，区分管理员和普通用户角色。
...
- **月报生成** — 基于日报调用大模型 API 自动生成月度工作总结，按项目/业务模块分类汇总
...
| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| GET | `/api/v1/monthly` | 查询所有月报 | ✓ |
| GET | `/api/v1/monthly/{year}/{month}` | 获取指定月报 | ✓ |
| POST | `/api/v1/monthly/{year}/{month}` | 生成/重新生成月报 | ✓ |
| PUT | `/api/v1/monthly/{year}/{month}` | 编辑月报内容 | ✓ |
